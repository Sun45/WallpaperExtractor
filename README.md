...
## 简介 🌟
Wallpaper Extractor 是一个专业的Steam创意工坊壁纸提取工具，基于JavaFX开发。通过实时监控Steam日志文件，自动检测Wallpaper Engine壁纸订阅状态变化，并提供便捷的壁纸提取功能。

## 功能 🚀
- 🔍 **实时监控**：自动监控Steam创意工坊日志文件变化，每秒检测一次
- 🖼️ **智能提取**：支持视频文件拷贝和整体文件夹拷贝两种模式
- ⚙️ **灵活配置**：可设置监控起始时间，支持自定义拷贝路径
- 🎨 **直观界面**：清晰的壁纸列表展示，实时显示订阅状态变化的壁纸
...
#### 2. 监控设置
- **起始时间**：设置监控的开始时间，只检测此时间后的壁纸变化
- **拷贝模式**：
  - **视频文件模式**：仅拷贝目录中的第一个视频文件（支持.mp4, .avi, .mov, .wmv, .flv, .mkv）
  - **文件夹模式**：完整拷贝整个壁纸文件夹结构

#### 3. 壁纸管理
- 检测到的壁纸订阅/取消订阅事件实时显示在主界面
- 每个壁纸项显示订阅时间、订阅状态和拷贝状态
- 手动触发壁纸批量提取操作
